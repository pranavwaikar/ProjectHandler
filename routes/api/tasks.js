...
router.put('/:id/start', auth, async (req, res) => {
  try {
    ...
    if (!tasks) {
      res
        .status(404)
        .send({ msg: 'No tasks under current project for this user!' });
    }
    ...
    if (task.type === TASK_TYPE.NORMAL_TASK && mandatoryTasks.length > 0) {
      res.status(403).send({ msg: 'please complete mandatory tasks first!' });
    }

    // if its already finished task then give error
    if (task.status === TASK_STATUS.FINISHED) {
      res.status(403).send({ msg: 'can not start finished task!' });
    }
    ...
    if (task.prerequisites) {
      for (let index = 0; index < task.prerequisites.length; index++) {
        ...
        if (testTask.status !== TASK_STATUS.FINISHED) {
          res
            .status(403)
            .send({ msg: 'please complete pre-requisite tasks first!' });
        }
      }
    }
  ...
  async (req, res) => {
    try {
      ...
      if (task.status === TASK_STATUS.NOT_STARTED) {
        res
          .status(403)
          .send({ msg: 'you need to start task before finishing!' });
      }
